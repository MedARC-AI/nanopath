...
All frozen feature extraction uses fp16 with batch 2048 and 16 decode workers, matching the settings already validated by the official driver for NanoPath ViT-S checkpoints on one 80 GB H100. Probe-head training remains batch 64. Runtime roots are the canonical shared locations declared in `configs/main.yaml`: `/data/thunder-data`, `/data/ucla-lung`, `/data/surgen`, `/data/leopard_bcr`, `/data/CPTAC-PDA`, and `/data/pathorob`. `prepare.py download=False` verifies every selected path, source-disjoint splits, and classification class counts.
