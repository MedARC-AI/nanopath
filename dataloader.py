...
class GPUAugment(nn.Module):
    def __init__(self, data, hue=0.0):
        super().__init__()
        self.hue = hue
    ...
    def forward(self, views):
        ...
        # Local hue joins the per-view random order; global crops keep three operations.
        hue_factors = (torch.rand(n, device=x.device) * 2 - 1) * self.hue * 2 * torch.pi if self.hue else None
        order = torch.rand(n, 4 if self.hue else 3, device=x.device).argsort(1)
        ...
        for position in range(order.shape[1]):
            ...
            mixed = torch.where(op == 0, brightness, torch.where(op == 1, contrast, saturation))
            x = torch.where(op == 3, K.enhance.adjust_hue(x, hue_factors), mixed) if self.hue else mixed
