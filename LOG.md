...
## 2026-04-30: LeJEPA objective on a DINOv2-init backbone (negative result) (@PaulScotti)
...
Implication for the leaderboard: there is no shortcut version of "DINOv2 init + LeJEPA fine-tune" that beats the existing leader. Continuing to use Meta's DINOv2 weights maybe only pays off when you also keep DINOv2's training objective (DINO CLS + iBOT).
